/* =========================================================
   CUSTOMER LEVEL REPORT JAVASCRIPT
   ========================================================= */
...
/* =========================================================
   EXPORT EXCEL LOGIC (GLOBAL DELEGATION)
   ========================================================= */
...
/* =========================================================
   FORMAT MENU & SUBMENUS (DELEGATED CLICK EVENTS)
   ========================================================= */
...
async function fetchCustomerReport(from, to) {
    const response = await fetch(`/api/customer-level?${new URLSearchParams({ from, to })}`);
...
async function loadComparisonReport() {
    ...
    const [previousReport, currentReportData] = await Promise.all([
        fetchCustomerReport(previousFrom, previousTo),
        fetchCustomerReport(currentFrom, currentTo)
    ]);
...
async function loadReport() {
    ...
    try {
        if (selectedFormat === "YTD" || selectedFormat === "MTD" || selectedFormat === "QTD") {
            ...
            const report = await fetchCustomerReport(fromDate.value, toDate.value);
        ...
        console.error("CUSTOMER LEVEL REPORT ERROR:", error);
        reportStatus.hidden = false;
        reportStatus.className = "report-status error";
...
/* =========================================================
   RENDER COMPARISON REPORT (Rowspan & Header Freeze Fix)
   ========================================================= */
...
function renderComparisonReport(report) {
    ...
    const tableHeader = `
        <thead>
            <tr>
                <th rowspan="2" class="${getSortClass('customer')} frozen-customer-header" onclick="changeSort('customer')" title="Sort Customer">
                    Customer
                </th>
            ...
            <tr>
                ...
                <th class="${getSortClass('last_year_percent')}" onclick="changeSort('last_year_percent')" title="Sort Last Year Sales %">Sales %</th>
                <th class="${getSortClass('last_year_running')}" onclick="changeSort('last_year_running')" title="Sort Last Year Running Sales %">Running %</th>
                <th class="${getSortClass('current_year')}" onclick="changeSort('current_year')" title="Sort Current Year Sales">Sales</th>
                <th class="${getSortClass('current_year_percent')}" onclick="changeSort('current_year_percent')" title="Sort Current Year Sales %">Sales %</th>
                <th class="${getSortClass('current_year_running')}" onclick="changeSort('current_year_running')" title="Sort Current Year Running Sales %">Running %</th>
            </tr>
        </thead>
    `;
...
/* =========================================================
   RENDER NORMAL / CUSTOM REPORT (Strict 2-Row Alignment)
   ========================================================= */
...
function renderReport(report) {
    ...
    const tableHeader = `
        <thead>
            <tr>
                <th rowspan="2" class="${getSortClass('customer')} frozen-customer-header" onclick="changeSort('customer')">Customer</th>
...
function initCustomerFilter() {
    const customerFilter = document.getElementById("customerFilter") || document.getElementById("customerSelect");
