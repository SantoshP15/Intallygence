...
OUTSTANDING_DEBTORS_COLUMNS = {
    "ledger": "LedgerName",
    "bill_ref": "bill_ref_no",
    "due_date": "bill_due_dt",
    "amount": "Netmount",
    "voucher_no": "vch_no",
    "voucher_date": "date",
...
def render_register_report(data_source, title, route_name):
    ...
    try:
                ...
                # Full Excel Export Logic (No LIMIT / OFFSET)
        if is_export:
            cursor.execute(
                f"""
                SELECT *
            ...
            # Primary key aur id columns hatayein
            cols_to_drop = [c for c in df.columns if c.lower() in ["primary_key", "id", "voucher_id"]]
            df.drop(columns=cols_to_drop, inplace=True, errors="ignore")

            # Column headers se underscores hatayein
            df.columns = [c.replace("_", " ") for c in df.columns]

            # In-memory Excel file build karein
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                # Excel sheet name limit 31 characters hoti hai
                sheet_title = title[:31]
                df.to_excel(writer, index=False, sheet_name=sheet_title)
            output.seek(0)

            safe_filename = f"{title.lower().replace(' ', '-')}.xlsx"
            return send_file(
                output,
        ...
        data = cursor.fetchall()
                # Primary key ko column list se exclude karein
        columns = [
            description[0]
            for description in cursor.description
            if description[0].lower() not in ["primary_key", "id", "voucher_id"]
        ]
...
def sales_vs_sales_return():
    ...
    for customer in sorted(year_values, key=str.casefold):
        ...
        rows.sort(key=lambda r: sum(y["returns"] for y in r["years"]), reverse=True)
    ...
    try:
        ...
        try:
            ...
            query = f"""
                ...
                FROM view_DayBook
                where {group_column}='Sundry Debtors'
                ORDER BY
                    {ledger_column},
                    {due_date_column},
                    {bill_column}
...
@app.route("/ledger-movement")
def ledger_movement():
    if "user" not in session:
        return redirect(url_for("splash"))

    return render_template("ledger-movement.html")


@app.route("/api/ledger-movement")
def ledger_movement_data():
...
def ageing_level():
    ...
    return render_template("ageing-level.html")


@app.route("/api/ageing-level")
def ageing_level_data():
